...
public class TravelController {
    ...
    public ResponseEntity<Object> updateTravel(@PathVariable(value = "id") UUID id,
        ...
        try {
            Travel travelModel = travel0.get();
            updateTravelFromDto(travelModel, travelDTO);
    ...
    private Travel toTravel(TravelDTO travelDTO) {
        Travel travel = new Travel();
        updateTravelFromDto(travel, travelDTO);
        return travel;
    }

    private void updateTravelFromDto(Travel travel, TravelDTO travelDTO) {
    ...
    private Date parseTime(String value) {
        ...
        return Date.from(localTime.atDate(TIME_ONLY_DATE).atZone(SYSTEM_ZONE).toInstant());
    }
