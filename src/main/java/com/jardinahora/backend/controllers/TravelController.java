...
public class TravelController {
    ...
    private void copyTravelDto(TravelDTO travelDTO, Travel travelModel) {
        ...
        travelModel.setStartTime(parseDateTime(travelDTO.startTime(), travelDate));
        travelModel.setEndTime(parseDateTime(travelDTO.endTime(), travelDate));
    }

    private Date parseDate(String date) {
        return toDate(LocalDate.parse(date, DATE_FORMATTER).atStartOfDay());
    }

    private Date parseDateTime(String value, LocalDate fallbackDate) {
        try {
            return toDate(LocalDateTime.parse(value, DATE_TIME_FORMATTER));
        } catch (DateTimeParseException ignored) {
            return toDate(LocalTime.parse(value, TIME_FORMATTER).atDate(fallbackDate));
        }
    }
