...
public class TravelController {
    ...
    private Date parseDate(String date) {
        ...
        return Date.from(localDate.atStartOfDay(APPLICATION_ZONE).toInstant());
    }

    private Date parseTime(String time) {
        LocalTime localTime = LocalTime.parse(time, TIME_FORMATTER);
        return Date.from(localTime.atDate(LocalDate.EPOCH).atZone(APPLICATION_ZONE).toInstant());
    }

    private String formatDate(Date date) {
        return DATE_FORMATTER.format(date.toInstant().atZone(APPLICATION_ZONE).toLocalDate());
    }
