...
public class TravelController {
    ...
    public ResponseEntity<Object> createTravel(@RequestBody @Valid TravelDTO travelDTO) {
        ...
        try {
            populateTravelModel(travelDTO, travelModel);
    ...
    private void populateTravelModel(TravelDTO travelDTO, Travel travelModel) {
        Date travelDate = parseDate(travelDTO.date());
        Date startTime = parseTime(travelDTO.startTime());
        Date endTime = parseTime(travelDTO.endTime());

        BeanUtils.copyProperties(travelDTO, travelModel, "date", "startTime", "endTime");
        travelModel.setDate(travelDate);
        travelModel.setStartTime(startTime);
        travelModel.setEndTime(endTime);
    }

    private Date parseDate(String value) {
        return java.sql.Date.valueOf(LocalDate.parse(value, DATE_FORMATTER));
    }

    private Date parseTime(String value) {
        return java.sql.Time.valueOf(LocalTime.parse(value, TIME_FORMATTER));
    }
