...
public class TravelController {
    ...
    public ResponseEntity<Object> createTravel(@RequestBody @Valid TravelDTO travelDTO) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Formato de data/hora inválido.");
        }
    }
    ...
    private static Date parseDate(String date) {
        return Date.from(LocalDate.parse(date, DATE_FORMATTER).atStartOfDay(ZONE_ID).toInstant());
    }

    private static Date parseTime(String time) {
        return Date.from(LocalTime.parse(time, TIME_FORMATTER).atDate(LocalDate.EPOCH).atZone(ZONE_ID).toInstant());
    }
